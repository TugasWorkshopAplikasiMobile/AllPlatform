...
(
   ...
   ID_ADMIN             varchar(6) not null,
   primary key (ID_NILAI_PSIKOTEST, ID_ADMIN)
);
...
(
   ...
   ID_ADMIN             varchar(6) not null,
   primary key (ID_NILAI_TEST, ID_ADMIN)
);
...
alter table ADMIN add constraint FK_VERIFIKASI foreign key (ID_PEMBAYARAN)
...
alter table DATA_TEMPAT_TINGGAL add constraint FK_MEMILIKI3 foreign key (ID_SISWA)
      references SISWA (ID_SISWA) on delete restrict on update restrict;
...
alter table INPUT add constraint FK_INPUT foreign key (ID_NILAI_PSIKOTEST)
      references NILAI_PSIKOTEST (ID_NILAI_PSIKOTEST) on delete restrict on update restrict;

alter table INPUT add constraint FK_INPUT3 foreign key (ID_ADMIN)
      references ADMIN (ID_ADMIN) on delete restrict on update restrict;

alter table INPUT2 add constraint FK_INPUT2 foreign key (ID_NILAI_TEST)
      references NILAI_TEST (ID_NILAI_TEST) on delete restrict on update restrict;

alter table INPUT2 add constraint FK_INPUT4 foreign key (ID_ADMIN)
      references ADMIN (ID_ADMIN) on delete restrict on update restrict;
...
alter table KESEHATAN_ANAK add constraint FK_MEMILIKI20 foreign key (ID_SISWA)
      references SISWA (ID_SISWA) on delete restrict on update restrict;

alter table KONDISI_KELUARGA add constraint FK_MEMILIKI21 foreign key (ID_SISWA)
      references SISWA (ID_SISWA) on delete restrict on update restrict;

alter table KUISINONER add constraint FK_MEMILIKI13 foreign key (ID_SISWA)
      references SISWA (ID_SISWA) on delete restrict on update restrict;

alter table MAPEL add constraint FK_MEMILIKI18 foreign key (ID_NILAI_TEST)
      references NILAI_TEST (ID_NILAI_TEST) on delete restrict on update restrict;

alter table MAPEL add constraint FK_MEMILIKI19 foreign key (ID_NILAI_PSIKOTEST)
      references NILAI_PSIKOTEST (ID_NILAI_PSIKOTEST) on delete restrict on update restrict;

alter table MENENTUKAN add constraint FK_MENENTUKAN foreign key (ID_HASIL)
      references HASIL (ID_HASIL) on delete restrict on update restrict;

alter table MENENTUKAN add constraint FK_MENENTUKAN2 foreign key (ID_ADMIN)
      references ADMIN (ID_ADMIN) on delete restrict on update restrict;

alter table NILAI_PSIKOTEST add constraint FK_MEMILIKI11 foreign key (ID_MAPEL)
      references MAPEL (ID_MAPEL) on delete restrict on update restrict;

alter table NILAI_PSIKOTEST add constraint FK_MEMILIKI23 foreign key (ID_SISWA)
      references SISWA (ID_SISWA) on delete restrict on update restrict;

alter table NILAI_TEST add constraint FK_MEMILIKI10 foreign key (ID_MAPEL)
      references MAPEL (ID_MAPEL) on delete restrict on update restrict;

alter table NILAI_TEST add constraint FK_MEMILIKI22 foreign key (ID_SISWA)
      references SISWA (ID_SISWA) on delete restrict on update restrict;

alter table PEMBAYARAN add constraint FK_MELAKUKAN2 foreign key (ID_SISWA)
      references SISWA (ID_SISWA) on delete restrict on update restrict;

alter table RIWAYAT_SEKOLAH add constraint FK_MEMILIKI2 foreign key (ID_SISWA)
      references SISWA (ID_SISWA) on delete restrict on update restrict;

alter table SAUDARA_KANDUNG_ANAK add constraint FK_MEMILIKI4 foreign key (ID_SISWA)
      references SISWA (ID_SISWA) on delete restrict on update restrict;
...
alter table SISWA add constraint FK_MEMILIKI12 foreign key (ID_KESEHATAN_ANAK)
      references KESEHATAN_ANAK (ID_KESEHATAN_ANAK) on delete restrict on update restrict;

alter table SISWA add constraint FK_MEMILIKI14 foreign key (ID_KONDISI_KELUARGA)
      references KONDISI_KELUARGA (ID_KONDISI_KELUARGA) on delete restrict on update restrict;

alter table SISWA add constraint FK_MEMILIKI15 foreign key (ID_NILAI_TEST)
      references NILAI_TEST (ID_NILAI_TEST) on delete restrict on update restrict;

alter table SISWA add constraint FK_MEMILIKI16 foreign key (ID_NILAI_PSIKOTEST)
      references NILAI_PSIKOTEST (ID_NILAI_PSIKOTEST) on delete restrict on update restrict;

alter table SISWA add constraint FK_MEMILIKI17 foreign key (ID_HASIL)
      references HASIL (ID_HASIL) on delete restrict on update restrict;
...
alter table USER add constraint FK_MEMILIKI5 foreign key (ID_SISWA)
      references SISWA (ID_SISWA) on delete restrict on update restrict;
